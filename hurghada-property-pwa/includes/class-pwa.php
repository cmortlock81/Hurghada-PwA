...
final class HPPWA_PWA {
    ...
    private function manifest(): void {
        ...
        echo wp_json_encode(['name'=>$s['app_name'],'short_name'=>$s['short_name'],'start_url'=>home_url('/app/'),'scope'=>home_url('/'),'display'=>'standalone','theme_color'=>$s['theme_colour'],'background_color'=>$s['background_colour'],'icons'=>[['src'=>HPPWA_URL.'assets/icons/icon-192.png','sizes'=>'192x192','type'=>'image/png','purpose'=>'any'],['src'=>HPPWA_URL.'assets/icons/icon-512.png','sizes'=>'512x512','type'=>'image/png','purpose'=>'any'],['src'=>HPPWA_URL.'assets/icons/maskable-192.png','sizes'=>'192x192','type'=>'image/png','purpose'=>'maskable'],['src'=>HPPWA_URL.'assets/icons/maskable-512.png','sizes'=>'512x512','type'=>'image/png','purpose'=>'maskable'],['src'=>HPPWA_URL.'assets/icons/icon.svg','sizes'=>'any','type'=>'image/svg+xml','purpose'=>'any'],['src'=>HPPWA_URL.'assets/icons/maskable.svg','sizes'=>'any','type'=>'image/svg+xml','purpose'=>'maskable']]], JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
    }
    private function service_worker(): void {
        ...
        $assets = [home_url('/app/'), home_url('/app/listings/'), home_url('/app/contact/'), HPPWA_URL.'assets/css/app.css', HPPWA_URL.'assets/js/app.js', HPPWA_URL.'assets/icons/icon-192.png', HPPWA_URL.'assets/icons/icon-512.png', HPPWA_URL.'assets/icons/maskable-192.png', HPPWA_URL.'assets/icons/maskable-512.png', HPPWA_URL.'assets/icons/icon.svg', HPPWA_URL.'assets/icons/maskable.svg'];
        ?>
...
self.addEventListener('push', event => { const data = event.data ? event.data.json() : {}; event.waitUntil(self.registration.showNotification(data.title || 'Hurghada Property PWA', { body: data.body || '', icon: <?php echo wp_json_encode(HPPWA_URL.'assets/icons/icon-192.png'); ?>, data: data.url || <?php echo wp_json_encode(home_url('/app/')); ?> })); });
